    ...
    public static void onPlayerLoggedIn(PlayerEvent.PlayerLoggedInEvent event) {
        if (PelmeniCraftConfig.showUpdates.get()) {
            ...
            if (updateCheckResult.status.equals(BETA_OUTDATED)) {
                ...
                String info = new TranslationTextComponent("chat.pelmenicraft.update_message").getString() + " \u00A7c" + currentVersion + "\u00A7r -> \u00A7a" + actualVersion + "\u00A7r";
                IFormattableTextComponent link = new TranslationTextComponent("chat.pelmenicraft.link").setStyle(Style.EMPTY.forceFormatting(TextFormatting.UNDERLINE).setClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, updateCheckResult.url)));

                IFormattableTextComponent message = new StringTextComponent(info).appendString(" [").appendSibling(link).appendString("]");
